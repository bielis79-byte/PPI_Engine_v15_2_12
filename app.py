...
st.set_page_config(page_title="VRC / HRV RRi Analyzer Pro v15.2.12 Inclusión multirregistro predictiva", layout="wide")
...
def infer_patient_id(record_name):
    """Infere una serie/paciente del prefijo del archivo, eliminando fecha y hora."""
...
def save_indices_to_longitudinal_db(indices_df):
    """Guarda/actualiza automáticamente cada registro-fase calculado, evitando duplicados."""
    init_longitudinal_db()
    if indices_df is None or indices_df.empty:
        return 0
    saved=0
    ...
    with sqlite3.connect(LONGITUDINAL_DB_PATH) as con:
        for _, row in indices_df.iterrows():
            record=str(row.get('Registro','')).strip()
            phase=str(row.get('Fase','')).strip()
            if not record or not phase:
                continue
            patient=infer_patient_id(record)
            dt=_extract_record_datetime(record)
            dt_text=dt.isoformat() if pd.notna(dt) else None
            key=hashlib.sha256(f'{patient}|{record}|{phase}'.encode('utf-8')).hexdigest()
            values=(
                key, patient, record, phase, dt_text, now,
                _safe_float_sql(row.get('IDX_Vagal')),
                _safe_float_sql(row.get('IDX_Amplitud')),
                _safe_float_sql(row.get('IDX_Complejidad')),
                _safe_float_sql(row.get('IDX_Rigidez')),
                _safe_float_sql(row.get('IDX_Adaptabilidad')),
                _safe_float_sql(row.get('IDX_Regulacion_Lenta')),
                _safe_float_sql(row.get('IDX_Confianza')),
                str(row.get('Perfil_autonomico','')),
            )
            con.execute('''
                INSERT INTO observations (
                    ...
                    IDX_Adaptabilidad, IDX_Regulacion_Lenta, IDX_Confianza, Perfil_autonomico
                ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                ON CONFLICT(observation_key) DO UPDATE SET
                    record_datetime=excluded.record_datetime,
                    saved_at=excluded.saved_at,
                    IDX_Vagal=excluded.IDX_Vagal,
                    IDX_Amplitud=excluded.IDX_Amplitud,
                    IDX_Complejidad=excluded.IDX_Complejidad,
                    IDX_Rigidez=excluded.IDX_Rigidez,
                    IDX_Adaptabilidad=excluded.IDX_Adaptabilidad,
                    IDX_Regulacion_Lenta=excluded.IDX_Regulacion_Lenta,
                    IDX_Confianza=excluded.IDX_Confianza,
                    Perfil_autonomico=excluded.Perfil_autonomico
            ''', values)
            saved += 1
        con.commit()
    return saved
...
def rr_plot(record_data, global_windows, record_windows, view_mode, selected_record, use_independent, pending_selection=None):
    fig = go.Figure()
...
st.title("VRC / HRV RRi Analyzer Pro v15.2.12 Inclusión multirregistro predictiva")
...
with st.sidebar:
    ...
    with st.expander("Historial persistente v15.2.12", expanded=True):
        auto_load_history_v152 = st.checkbox(
            "Cargar automáticamente registros guardados",
            value=True,
            help="Recupera RRi y segmentaciones anteriores sin volver a subir TXT/CSV."
        )
        ...
        selected_saved_keys_v152 = st.multiselect(
            "Registros históricos a cargar",
            _catalog_options_v152,
            default=_catalog_options_v152 if auto_load_history_v152 else [],
            format_func=lambda k: _catalog_labels_v152.get(str(k), str(k)),
            disabled=not auto_load_history_v152,
        )
        ...
        if LONGITUDINAL_DB_PATH.exists():
            st.download_button(
                ...
                file_name="base_completa_v1522.sqlite3",
                mime="application/octet-stream",
                key="download_full_db_v152_sidebar",
            )
...
def predict_v15_continuous(history_df, patient_id, phase):
    ...
    return {'tabla':table,'estado_previsto':state,'compuesto_actual':c0,'compuesto_predicho':c1,
            ...
            'anomalia_score':anomaly,'anomalia_label':anomaly_label,'history':df,
...
def predict_v15_all_phases(history_df, patient_id):
    ...
    detail_rows = []
    for phase in phases:
        pred = predict_v15_continuous(history_df, patient_id, phase)
        if pred is None:
            continue
...
# v15.2 · Guarda automáticamente las ventanas efectivas de cada registro.
try:
...
with tab1:
    st.subheader("Segmentación tipo Kubios · selección independiente por registro v15.2.12")
    ...
    if loaded_from_history_v152:
        st.success("RRi y segmentaciones anteriores recuperados. Los resultados se recalculan automáticamente.")
    elif not _saved_catalog_v152.empty:
        st.info("Hay registros guardados. Actívalos desde «Historial persistente v15.2.12» en la barra lateral.")
    st.write(
    ...
    with c1:
        view_mode = st.radio("Vista", ["Registro principal", "Todos superpuestos", "Paneles independientes"], index=2)
    with c2:
    ...
    if view_mode == "Paneles independientes":
        ...
        _panel_records = list(records)
        _panel_cols = st.columns(2) if len(_panel_records) > 1 else [st.container()]
        ...
        for _idx, _rec in enumerate(_panel_records):
            with _panel_cols[_idx % len(_panel_cols)]:
                ...
                _event_rec = st.plotly_chart(
                    _fig_rec,
                    use_container_width=True,
                    on_select="rerun",
                    selection_mode=("box", "lasso"),
                    key=f"rr_select_independent_{hashlib.md5(_rec.encode()).hexdigest()[:10]}",
                )

                if _event_rec and getattr(_event_rec, "selection", None):
                    _pts = _event_rec.selection.get("points", [])
                    _xs = [p.get("x") for p in _pts if p.get("x") is not None]
                    if _xs:
                        _s, _e = min(_xs) * 60.0, max(_xs) * 60.0
                        _dur = float(record_data[_rec].get("duration", 0.0))
                        _s = max(0.0, min(_s, _dur))
                        _e = max(0.0, min(_e, _dur))
                        if _e > _s:
                            _new_sel = [_s, _e]
                            ...
                            if _old_sel is None or not np.allclose(_old_sel, _new_sel, atol=0.25):
                                ...
                                save_temporal_selection(
                                    _rec, record_data[_rec].get("filename", _rec), _s, _e
                                )
                                st.rerun()
                ...
                if _pending_rec is not None:
                    _s, _e = _pending_rec
                    st.success(
                        f"Selección: {sec_to_hms(_s)} – {sec_to_hms(_e)} "
                        f"({sec_to_hms(_e - _s)})"
                    )
...
with tab10:
    st.subheader("Motor de predicción fisiológica continua v15.2.11 · inclusión multirregistro")
    st.info(
    ...
    if not _patient_pool_v1529:
        ...
        if source_mode.startswith('Solo'):
            ...
            if _missing:
                st.warning(
                    f"{len(_missing)} archivo(s) cargado(s) no alcanzan el mínimo de {int(min_rr)} RRi "
                    "o no han podido generar índices válidos: " + ", ".join(_missing)
                )
            else:
        ...
        if _patient_history_v1529.empty:
            ...
            if summary15.empty:
                ...
                st.download_button(
                    ...
                    file_name=f'prediccion_multifase_v15211_{sanitize_name(pa)}.csv',mime='text/csv'
                )
                st.download_button(
                    'Descargar detalle de todos los índices y fases CSV',details15.to_csv(index=False).encode('utf-8'),
                    file_name=f'prediccion_indices_multifase_v15211_{sanitize_name(pa)}.csv',mime='text/csv'
                )
    ...
    with st.expander('Cómo funciona el motor v15.2.9',expanded=True):
        st.markdown("""
...
9. **Estado resumen:** deriva Favorable/Estable/Desfavorable sólo después de predecir los índices; no se utiliza como requisito de entrenamiento.
